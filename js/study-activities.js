...
export async function renderLaunch(root, id){
  const groups = await API.getGroups(); const acts = await API.getActivities(); const a = acts.find(x=>x.id==id) || acts[0];
  root.innerHTML = `
    ...
    <div class="card p-6 mt-4 max-w-xl">
      ...
      <p class="text-xs text-zinc-500 mt-3">Will POST /api/study_activities and open activity URL in new tab, then redirect to session show.</p>
    </div>
  `;
  document.getElementById('launchBtn').onclick = ()=>{
    ...
    // mock POST
    console.log('POST /api/study_activities', {activity_id:id, group_id:gid});
    location.hash = '#/study_sessions/999';
  };
}
